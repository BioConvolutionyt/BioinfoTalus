...
BioinfoTalus 是一个面向生物信息学转录组数据分析的 R 脚本集合，覆盖 **Bulk 转录组**、**单细胞转录组** 与 **空间转录组**三大方向，包含常见算法与分析流程的可复用实现与示例数据。

### 🎯项目特点

- **既可作为一套完整 pipeline，也可独立使用**：脚本之间通过 `Data/` 下的中间产物（如 `.Rda`、`.txt`、`.csv`）衔接；同时每个脚本也尽量设计为“拿来即用”的独立模块。
- **可泛化 + 注释较完整**：核心步骤与关键参数已尽量写成可迁移的形式，并对需要按数据调整的部分进行了标注；尤其是**数据读取/预处理**相关位置可能需要你根据实际数据结构做少量修改。
- **内置示例数据与部分运行结果**：`Data/` 目录包含示例数据，以及部分脚本运行后生成的结果文件，方便快速上手与复现。
...
- **`Data/`**：示例数据、参考资源与部分运行结果的存放文件夹，中间文件与输出文件默认也会写回此目录，数据文件具体内容存放在GitHub Releases（见数据文件下载）；

### 💡示例数据说明

- **Bulk 转录组**：示例使用 **TCGA-HNSC** 数据集（表达矩阵/临床信息等按脚本要求放置于 `Data/TCGA/` 相关目录）；
- **单细胞转录组（10x）**：示例来自 **GSM9113377、GSM9113378、GSM9113378**，为**结直肠癌**样本（位于 `Data/10x Single Cell/`）；
- **空间转录组（10x Visium）**：示例来自 **GSM8594561**，同为**结直肠癌**样本（位于 `Data/10x Visium/`）；
  - 注意：空间示例与单细胞示例**不配对**（可用于演示流程与方法，但不代表严格的配对设计）；

### 🧩数据文件下载

项目提供的示例数据、参考资源与部分运行结果文件较大，请从 GitHub Releases 下载：
...
- **Bulk（TCGA 示例）**
  ...
  - 其他：`Cibersort_main.R`（免疫浸润）、`WGCNA.R`、`药物敏感性分析.R`、`免疫检查点分析.R`、`TCGA体细胞突变数据整理.R` 等按需选用

- **单细胞（10x 示例，Seurat）**
  - `单细胞转录组测序数据分析/单细胞测序数据读取与质控.R`
  - `单细胞转录组测序数据分析/单细胞测序数据降维、聚类、去除批次效应.R`
  - `单细胞转录组测序数据分析/细胞注释.R`（会写入 `pbmc$celltype` 等信息）
  - 其他模块：`双细胞去除.R`、`细胞周期去除.R`、`inferCNV.R`、`CopyKat.R`、`拟时序分析（monocle2）.R`、`拟时序分析（monocle3）.R`、`Slingshot.R`、`CytoTRACE2.R`、`细胞通讯.R`、`AUCell.R`、`hdWGCNA.R` 等

- **空间转录组（Visium 示例，Seurat Spatial）**
  - `空间转录组测序数据分析/空间转录组数据处理.R`
  - `空间转录组测序数据分析/反卷积.R`（需要单细胞对象的细胞类型先验，示例使用 `Data/pbmc.Rda`）
  - `空间转录组测序数据分析/空间差异基因分析.R`（SPARK）
  - `空间转录组测序数据分析/空间细胞通讯分析.R`（CellChat spatial）

#### 独立运行某个脚本

大多数脚本在开头会 `load(...)` 或 `read...(...)` 指定其所需输入（通常位于 `Data/`）。若你希望将其用于自己的数据：

- 按脚本注释准备输入文件/对象（路径、列名、分组信息、注释信息等）；
- 根据标注位置调整少量参数（例如 QC 阈值、`pcs`、`resolution`、基因集路径、分组 cutpoint 等）；
