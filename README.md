...
- **`Data/`**：示例数据、参考资源与部分运行结果的存放文件夹，中间文件与输出文件默认也会写回此目录，为避免仓库体积过大，Git 仓库中 `Data/` 默认为空，数据文件具体内容存放在GitHub Releases（见数据文件下载）；

### 💡示例数据说明

- **Bulk 转录组**：示例使用 **TCGA-HNSC** 数据集（下载数据后，按脚本要求放置于 `Data/TCGA/` 相关目录）；
- **单细胞转录组（10x）**：示例来自 **GSM9113377、GSM9113378、GSM9113378**，为**结直肠癌**样本（下载数据后位于 `Data/10x Single Cell/`）；
